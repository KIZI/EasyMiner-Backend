...
import cz.vse.easyminer.preprocessing.{ValueDetail => PreprocessingValueDetail, _}
...
trait PreprocessingDbOps {
  ...
  def buildDataset(dataSourceName: String, data: InputStream, dataTypes: List[Option[FieldType]] = Nil): DatasetDetails = new DatasetDetails(
    datasetTypeOps.toDatasetOps.getAllDatasets.find(x => x.name == dataSourceName && dataSourceTypeOps.toDataSourceOps.getDataSource(x.dataSource).nonEmpty).getOrElse {
      ...
      val attributes = dataSourceTypeOps.toFieldOps(dataSourceDetail).getAllFields.map(field => SimpleAttribute(field.name, field.id, Nil))
      datasetTypeOps.toAttributeBuilder(datasetDetail, attributes: _*).build
      datasetDetail
    }
  )
  ...
  class DatasetDetails private[PreprocessingDbOps](val datasetDetail: DatasetDetail) {
    ...
    val instanceTable = datasetDetail.`type` match {
      case LimitedDatasetType => new MysqlInstanceTable(datasetDetail.id)
      case UnlimitedDatasetType => new HiveInstanceTable(datasetDetail.id)
    }
    ...
    val valueMap: Map[String, Map[String, PreprocessingValueDetail]] = attributeMap.map { case (key, attributeDetail) =>
      key -> datasetTypeOps.toValueOps(datasetDetail, attributeDetail).getValues(0, 1000).map(x => x.value -> x).toMap
    }

    def colId(attributeName: String) = attributeMap(attributeName).id
...
trait MysqlPreprocessingDbOps extends PreprocessingDbOps {
  ...
  protected[this] lazy val dataSourceTypeOps: DataSourceTypeOps[_] = limitedDataSourceTypeToMysqlDataSourceTypeOps(LimitedDataSourceType)
...
trait HivePreprocessingDbOps extends PreprocessingDbOps {
  ...
  protected[this] lazy val dataSourceTypeOps: DataSourceTypeOps[_] = unlimitedDataSourceTypeToHiveDataSourceTypeOps(UnlimitedDataSourceType)
  protected[this] lazy val datasetTypeOps: DatasetTypeOps[_] = unlimitedDatasetTypeToHiveDatasetTypeOps(UnlimitedDatasetType)
