...
class MineSpec extends FlatSpec with Matchers with ConfOpt with TemplateOpt {
  ...
  implicit val rcp = RConnectionPoolImpl.default
  ...
  "R Script with UTF8+space select query and consequents" should "return one association rule" in {
    ...
    RScript evalTx { r =>
      val template = rscript(
        ...
        s"IF(${colName("district")}=${valueId("district", "Jindřichův Hradec")} OR ${colName("district")}=${valueId("district", "Jihlava")}, ${colName("district")}, NULL) AS ${colName("district")}, IF(${colName("rating")}=${valueId("rating", "A")} OR ${colName("rating")}=${valueId("rating", "B")}, ${colName("rating")}, NULL) AS ${colName("rating")}",
        "lhs",
        Some( s""" "${colName("rating")}=${valueId("rating", "A")}","${colName("rating")}=${valueId("rating", "B")}" """.trim),
        None,
    ...
    RScript evalTx { r =>
      val template = rscript(
        instanceTable.tableName,
        s"IF(${colName("district")}=${valueId("district", "Jihlava")}, ${colName("district")}, NULL) AS ${colName("district")}, IF(${colName("rating")}=${valueId("rating", "C")}, ${colName("rating")}, NULL) AS ${colName("rating")}",
        "lhs",
        Some( s""" "${colName("rating")}=${valueId("rating", "C")}" """.trim),
        None,
  ...
  "R Script with small support and confidence" should "return many results" in {
    ...
    RScript.evalTx { r =>
      val template = rscript(
        ...
        "*",
        "lhs",
        Some( s""" "${colName("rating")}=${valueId("rating", "A")}","${colName("rating")}=${valueId("rating", "B")}","${colName("rating")}=${valueId("rating", "C")}","${colName("rating")}=${valueId("rating", "D")}" """.trim),
        None,
  ...
  it should "throw an exception due to bad attributes for CBA" in {
    ...
    val badAttributes: Seq[BoolExpression[Attribute]] = Seq(
      ...
      Value(AllValues(attributeMap("rating"))) AND Value(FixedValue(attributeMap("age"), NormalizedValue(valueId("age", "51")))),
      Value(*)
    )
  ...
  it should "mine with an empty antecedent or consequent" in {
    ...
    val emptyConsequentResult = RScript.evalTx { r =>
      val task = MinerTask(datasetDetail, Some(Value(FixedValue(attributeMap("age"), NormalizedValue(valueId("age", "25"))))), InterestMeasures(Support(0.001), Confidence(0.01), Limit(200), MinRuleLength(1), MaxRuleLength(8)), None)
      miner(r).mine(task)(_ => {}).rules
    }
    emptyConsequentResult.size shouldBe 105
    emptyConsequentResult.forall(_.antecedent match {
      case Some(Value(FixedValue(attr, value))) if attr == attributeMap("age") && value.value == valueId("age", "25") => true
      case None => true
      case _ => false
    }) shouldBe true
    ...
    emptyAntecedentResult.forall(_.consequent match {
      case Value(FixedValue(attr, _)) if attr == attributeMap("rating") => true
      case _ => false
    }) shouldBe true
  ...
  it should "be able to mine with a same attribute in both sides" in {
    ...
    val result = RScript.evalTx { r =>
      val task = MinerTask(datasetDetail, Some(Value(AllValues(attributeMap("rating"))) AND Value(FixedValue(attributeMap("age"), NormalizedValue(valueId("age", "31"))))), InterestMeasures(Support(0.001), Confidence(0.04), Limit(200), MinRuleLength(1), MaxRuleLength(8)), Some(Value(AllValues(attributeMap("rating"))) AND Value(AllValues(attributeMap("age"))) AND Value(FixedValue(attributeMap("district"), NormalizedValue(valueId("district", "Praha"))))))
      miner(r).mine(task)(_ => {}).rules
    }
  ...
  it should "mine with auto param" in {
    ...
    RScript.evalTx { r =>
      val task = MinerTask(datasetDetail, Some(Value(AllValues(attributeMap("district"))) AND Value(AllValues(attributeMap("age")))), InterestMeasures(Limit(100), Auto), Some(Value(FixedValue(attributeMap("rating"), NormalizedValue(valueId("rating", "A")))) OR Value(FixedValue(attributeMap("rating"), NormalizedValue(valueId("rating", "B"))))))
      miner(r).mine(task)(_ => {}).rules.size should be > 0
    }
  }
