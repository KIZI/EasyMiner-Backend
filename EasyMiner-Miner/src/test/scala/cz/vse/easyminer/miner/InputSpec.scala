...
import cz.vse.easyminer.miner.impl.PmmlTaskParser.Exceptions.NoDataset
import cz.vse.easyminer.miner.impl.{MinerTaskValidatorImpl, PmmlTaskParser}
...
class InputSpec extends FlatSpec with Matchers with TemplateOpt {
  ...
  "PMML Task" should "have antecedent and consequent" in {
    ...
    Inside.inside(minerTask.consequent) {
      case Some(Value(FixedValue(x, y))) if x.name == "rating" && x.uniqueValuesSize == 4 =>
        datasetDetail.`type`.toValueMapperOps(datasetDetail).normalizedValueMapper(Map(x -> Set(y))).value(x, y) should matchPattern {
  ...
  it should "not be valid" in {
    ...
    intercept[BadInterestMeasureInput] {
      ...
      validator.validate(minerTask2.copy(consequent = Some(minerTask2.consequent.get AND Value(FixedValue(fixedValue.attributeDetail.copy(id = 999), NormalizedValue(999))))))
    }
