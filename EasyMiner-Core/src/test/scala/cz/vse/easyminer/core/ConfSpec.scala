...
class ConfSpec extends FlatSpec with Matchers with ConfOpt {
  ...
  "Conf" should "return strings for test db settings" in {
    ...
    val conntimeout = Conf().getOrElse[Duration]("easyminer.db.connection-timeout", Duration.Inf)
    if (conntimeout.isFinite()) {
      conntimeout.toMillis should be > 0L
    } else {
      conntimeout shouldBe Duration.Inf
    }
  }
