...
/**
  ...
  */
class MysqlNominalEnumerationAttributeBuilder private(val dataset: DatasetDetail,
                                                      val attributes: Seq[NominalEnumerationAttribute])
                                                     (implicit
                                                      protected val mysqlDBConnector: MysqlDBConnector,
                                                      protected val taskStatusProcessor: TaskStatusProcessor) extends DbAttributeBuilder[NominalEnumerationAttribute] with DbMysqlTables {
  ...
  protected def buildAttributes(attributes: Seq[AttributeWithDetail]): Seq[AttributeDetail] = {
    ...
    def getSelectCond(valueCol: SQLSyntax, fieldCol: SQLSyntax) = attributes.foldLeft(sqls"null") { (select, attributeWithDetail) =>
      val uncoveredValue = if (attributeWithDetail.attribute.features.contains(PreserveUncovered)) valueCol else sqls"null"
    ...
    DBConn autoCommit { implicit session =>
      ...
      val selectStage2 = sqls"SELECT s1.a, s1.nom, SUM(s1.f) FROM ($selectStage1) s1 WHERE s1.nom IS NOT NULL GROUP BY s1.a, s1.nom"
