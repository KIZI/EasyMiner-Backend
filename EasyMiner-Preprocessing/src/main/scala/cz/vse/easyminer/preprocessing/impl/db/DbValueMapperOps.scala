...
import cz.vse.easyminer.data.{NullValue, NumericValue, NominalValue, Value}
import cz.vse.easyminer.preprocessing.ValueMapperOps.{ValueMapper, NormalizedValueMapper}
...
trait DbValueMapperOps extends ValueMapperOps {
   ...
   * @return SQLSyntax
   */
  implicit private def buildValuesWhereFetchQuery(normalizedValues: Values[NormalizedValue]): SQLSyntax = normalizedValues.map { case (attributeDetail, normalizedValueSeq) =>
    val ids = normalizedValueSeq.map(_.value)
    sqls"${valueDetailTable.column.attribute} = ${attributeDetail.id} AND ${valueDetailTable.column.id} IN ($ids)"
  }.reduce(_ or _)
  ...
  implicit private def buildNormalizedValuesWhereFetchQuery(values: Values[Value]): SQLSyntax = values.map { case (attributeDetail, valueSeq) =>
    val preparedValues = valueSeq.collect {
      ...
      case NumericValue(value) => Some(value)
    }
    val valueColumn = attributeDetail.`type` match {
      case NominalAttributeType => valueDetailTable.column.valueNominal
      case NumericAttributeType => valueDetailTable.column.valueNumeric
    }
    sqls"${valueDetailTable.column.attribute} = ${attributeDetail.id} AND ($valueColumn IN ($preparedValues) OR $valueColumn IS NULL)"
  }.reduce(_ or _)
  ...
  private def fetchValueDetails(whereSyntax: SQLSyntax)(implicit dBSession: DBSession, attributeMap: Map[Int, AttributeDetail]) = {
    sql"SELECT ${valueDetailTable.column.*} FROM ${valueDetailTable.table} WHERE $whereSyntax".map { wrs =>
      ...
      val value: Option[ValueDetail] = attribute.`type` match {
        ...
        case NumericAttributeType => wrs.doubleOpt(valueDetailTable.column.valueNumeric).map(value => NumericValueDetail(id, attribute.id, value, frequency))
      }
  ...
  private trait MapperPairBuilder[A, B] {
    def buildMapperItem(attributeDetail: AttributeDetail, value: Value, normalizedValue: NormalizedValue): (A, B)
  }

  implicit private object ValueMapperPairBuilder extends MapperPairBuilder[MapperKey[Value], NormalizedValue] {
    def buildMapperItem(attributeDetail: AttributeDetail, value: Value, normalizedValue: NormalizedValue): (MapperKey[Value], NormalizedValue) = (attributeDetail, value) -> normalizedValue
  }

  implicit private object NormalizedValueMapperPairBuilder extends MapperPairBuilder[MapperKey[NormalizedValue], Value] {
    def buildMapperItem(attributeDetail: AttributeDetail, value: Value, normalizedValue: NormalizedValue): (MapperKey[NormalizedValue], Value) = (attributeDetail, normalizedValue) -> value
  }
  ...
  private def valueDetailToMapperPair[A, B](valueDetail: ValueDetail)(implicit attributeMap: Map[Int, AttributeDetail], mapperPairBuilder: MapperPairBuilder[A, B]): (A, B) = {
    ...
    val value = valueDetail match {
      ...
      case v: NumericValueDetail => NumericValue(v.value)
      case _: NullValueDetail => NullValue
    }
    val normalizedValue = NormalizedValue(valueDetail.id)
   ...
   */
  class LoadedValueMapper(mapper: Map[MapperKey[Value], NormalizedValue]) extends ValueMapper {
    def normalizedValue(attributeDetail: AttributeDetail, value: Value): Option[NormalizedValue] = mapper.get(attributeDetail -> value)
  }
   ...
   * @param mapper loaded mapper
   */
  class LoadedNormalizedValueMapper(mapper: Map[MapperKey[NormalizedValue], Value]) extends NormalizedValueMapper {
    def value(attributeDetail: AttributeDetail, normalizedValue: NormalizedValue): Option[Value] = mapper.get(attributeDetail -> normalizedValue)
  }
  ...
  def normalizedValueMapper(normalizedValues: Values[NormalizedValue]): NormalizedValueMapper = new LoadedNormalizedValueMapper(buildMapper(normalizedValues))
